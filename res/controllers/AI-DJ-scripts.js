...
var AIDJ = (function() {
    ...
    api.sendWire = function(message, deadline) {
        ...
        return wireEndpoint.send(message, deadline);
    };
    api.cancelWireSend = function(id) { return wireEndpoint ? wireEndpoint.cancelSend(id) : false; };
...
// Fragment: wire-send.js
// Autonomously AI-generated by scripts/build-host-sender.mjs; do not edit.
// Node sender SHA-256: 5797192577a3da9c1f7d5a88b1b52ed7992d8513a7a570ee698f3afeb08dc848
(function(){
"use strict";
...
const FRAGMENT_SEND_LIMITS = Object.freeze({ messages: 4, bytes: 330000, results: 32,
...
function createFragmentSender(options) {
    if (!Number.isSafeInteger(options.generation) || options.generation < 0 || typeof options.now !== "function" ||
        ...
        (options.allowDiagnostic !== undefined && typeof options.allowDiagnostic !== "boolean"))
        throw Error("Invalid fragment sender options");
    const generation = options.generation, diagnostic = options.allowDiagnostic === true;
    const queue = [], results = [];
    let closed = false, fault = null, busy = false, lastClock = -1, nextSend = 0, retained = 0;
    let timer;
    ...
    function clock() {
        ...
        if (!Number.isFinite(now) || now < 0 || now > Number.MAX_SAFE_INTEGER || now < lastClock) {
            fault = "clock";
            retire("fault");
            return null;
        }
    ...
    function pump() {
        ...
        for (const job of [...queue]) {
            if (now >= job.deadline || (job.first !== null && now - job.first >= FRAGMENT_SEND_LIMITS.assemblyMs) ||
                (job.last !== null && now - job.last >= FRAGMENT_SEND_LIMITS.idleMs))
                finish(job, "expired");
        }
        const job = queue[0];
        if (!job)
        ...
        nextSend = now + FRAGMENT_SEND_LIMITS.intervalMs;
        busy = true;
        const frame = job.frames[job.sent];
        job.attempted++;
        try {
            const returned = options.send(Array.from(frame));
            if (returned !== undefined) {
                if (returned instanceof Promise)
                    void returned.catch(() => { });
                throw Error("Asynchronous or value-returning MIDI writer");
            }
            job.sent++;
            (_a = job.first) !== null && _a !== void 0 ? _a : (job.first = now);
            job.last = now;
        }
        catch (_b) {
            fault = "send-outcome-unknown";
            retire("fault");
        }
        finally {
            busy = false;
        ...
        if (job.stop)
            finish(job, job.stop);
        else if (job.sent === job.frames.length)
            finish(job, "sent");
        schedule();
    }
    return {
        enqueue(id, message, deadline, inputGeneration) {
            if (busy)
                throw Error("Reentrant enqueue");
            if (closed)
                throw Error("Fragment sender closed");
            if (inputGeneration !== generation)
                throw Error("Stale generation");
            if (typeof id !== "string" || !/^[A-Za-z0-9_.:-]{1,128}$/.test(id) || queue.some(j => j.id === id))
                throw Error("Invalid or pending duplicate id");
            const now = clock();
            if (now === null)
                throw Error("Fragment sender clock failed");
            if (!Number.isFinite(deadline) || deadline <= now || deadline > Number.MAX_SAFE_INTEGER)
                throw Error("Invalid deadline");
            if (queue.length >= FRAGMENT_SEND_LIMITS.messages || queue.length + results.length >= FRAGMENT_SEND_LIMITS.results)
                throw Error("Fragment sender capacity");
            const frames = AIDJ.encodeWire(message, diagnostic).map(f => Uint8Array.from(f)), bytes = frames.reduce((n, f) => n + f.length, 0);
            if (retained + bytes > FRAGMENT_SEND_LIMITS.bytes)
                throw Error("Fragment byte capacity");
            queue.push({ id, frames, bytes, deadline, attempted: 0, sent: 0, first: null, last: null, stop: null });
            retained += bytes;
            schedule();
        },
        cancel(id) { const job = queue.find(j => j.id === id); if (!job)
            return false; job.stop = "cancelled"; if (!busy || job !== queue[0])
            finish(job, "cancelled"); return true; },
        pump,
        close() { retire("closed"); },
