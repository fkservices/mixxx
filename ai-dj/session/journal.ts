...
export class SessionJournal {
 ...
 constructor(options:{directory:string;recordingId:string;maxRecords?:number;maxBytes?:number;timeoutMs?:number}) {
  ...
  this.worker.on("message",(m:{kind:string;id?:number;reason?:string;hasGap?:boolean})=>{
 ...
 private fail(reason:string){if(this.state==="failed"||this.state==="closed")return;this.clearTimer();this.state="failed";this.reason=reason;this.notSubmitted=this.queue.filter(p=>p.kind==="event").length;this.unconfirmed=this.flight?.packet.kind==="event"?1:0;this.queue=[];this.flight=null;this.bytes=0;this.readyReject(new Error(reason));this.closeReject?.(new Error(reason));void this.worker.terminate();}
 private pump(){
