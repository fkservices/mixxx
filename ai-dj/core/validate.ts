...
import { ACTION_LIMITS, SYNC_ACTION_LIMITS, type ActionOutcome, type MomentaryReleaseObligation, type SemanticAction } from "./actions.ts";
...
function controlResource(value: unknown, path: string, sync = false): string {
  if (value === null || typeof value !== "object" || Array.isArray(value)) issue(path, "expected-object", "Expected control resource");
  const kind = (value as JsonObject).kind;
  if (sync) {
    if (kind === "sync-engine") { object(value, path, ["kind"]); return "sync-engine"; }
    const r = object(value, path, ["kind", "deckId"]);
    literal(r.kind, "deck-sync", `${path}.kind`);
    return `deck-sync:${id(r.deckId, `${path}.deckId`)}`;
  }
...
function authority(value: unknown, path: string, sync = false): { kind: "performance" | "release-cleanup"; ownership: Set<string>; cleanupLease?: { obligationId: string; pressActionId: string; pressGeneration: number } } {
  if (value === null || typeof value !== "object" || Array.isArray(value)) issue(path, "expected-object", "Expected authority");
  if ((value as JsonObject).kind === "release-cleanup") { if (sync) issue(path, "invalid-authority-combination", "Sync requires performance authority"); const r = object(value, path, ["kind", "lease"]); return { kind: "release-cleanup", ownership: new Set(), cleanupLease: lease(r.lease, `${path}.lease`) }; }
  ...
  for (const [index, claim] of array(r.ownership, `${path}.ownership`, ACTION_LIMITS.maxOwnershipClaims).entries()) {
    ...
    const resource = controlResource(c.resource, `${path}.ownership[${index}].resource`, sync); safeInteger(c.ownershipGeneration, `${path}.ownership[${index}].ownershipGeneration`); id(c.grantId, `${path}.ownership[${index}].grantId`);
...
function freshness(value: unknown, path: string, sync = false): Map<string, { binding: "empty" | "loaded" }> {
  ...
  if (!sync) conditions(r.conditions, `${path}.conditions`, new Set(decks.keys()));
  return decks;
}
...
function actionVersion(value: unknown, path: string): void {
  if (value !== 1 && value !== 2) issue(path, "invalid-literal", "Expected action schema version 1 or 2");
}

/** Checks internal consistency only; retained observations/topology/grants need executor resolution. */
function syncOperation(value: unknown, guards: unknown, decks: Map<string, { binding: "empty" | "loaded" }>, ownership: Set<string>): void {
  const op = object(value, "$.operation", ["kind", "action", "deckId", "args"]);
  literal(op.kind, "desired-state", "$.operation.kind");
  literal(op.action, "deck.set_sync_enabled", "$.operation.action");
  const target = id(op.deckId, "$.operation.deckId");
  bool(object(op.args, "$.operation.args", ["enabled"]).enabled, "$.operation.args.enabled");
  if (decks.get(target)?.binding !== "loaded") issue("$.operation.deckId", "unknown-or-empty-binding", "Sync target requires a declared loaded binding");
  const entries = array(guards, "$.preconditions.conditions", 1);
  if (entries.length !== 1) issue("$.preconditions.conditions", "missing-sync-context", "Exactly one sync context is required");
  const path = "$.preconditions.conditions[0]";
  const g = object(entries[0], path, ["kind", "topologyEvidenceId", "topologyRevision", "participants", "leader", "leaderObservationId", "internalClockBpm", "pendingSyncChange", "effectPolicy"]);
  literal(g.kind, "sync-context", `${path}.kind`);
  id(g.topologyEvidenceId, `${path}.topologyEvidenceId`);
  safeInteger(g.topologyRevision, `${path}.topologyRevision`);
  const observations = new Set<string>();
  const observation = (v: unknown, p: string) => {
    const key = id(v, p);
    if (observations.has(key)) issue(p, "duplicate-observation", "Each sync fact requires a distinct observation");
    observations.add(key);
  };
  const expected = (v: unknown, p: string, check: (v: unknown, p: string) => unknown): unknown => {
    const r = object(v, p, ["value", "observationId"]);
    observation(r.observationId, `${p}.observationId`);
    check(r.value, `${p}.value`);
    return r.value;
  };
  const range = (minimum: number, maximum: number, openMin = false, openMax = false) => (v: unknown, p: string) => {
    const n = number(v, p);
    if (n < minimum || n > maximum || (openMin && n === minimum) || (openMax && n === maximum)) issue(p, "out-of-range", "Sync value is outside its contract bounds");
  };
  const falseValue = (v: unknown, p: string) => literal(v, false, p);
  const trueValue = (v: unknown, p: string) => literal(v, true, p);
  const bpm = range(0, SYNC_ACTION_LIMITS.maxBpm, true);
  observation(g.leaderObservationId, `${path}.leaderObservationId`);
  expected(g.internalClockBpm, `${path}.internalClockBpm`, bpm);
  expected(g.pendingSyncChange, `${path}.pendingSyncChange`, falseValue);
  const policy = object(g.effectPolicy, `${path}.effectPolicy`, ["policyId", "maxTempoChangeRatio", "maxPhaseDisplacementBeats"]);
  id(policy.policyId, `${path}.effectPolicy.policyId`);
  range(0, SYNC_ACTION_LIMITS.maxTempoChangeRatio)(policy.maxTempoChangeRatio, `${path}.effectPolicy.maxTempoChangeRatio`);
  range(0, SYNC_ACTION_LIMITS.maxPhaseDisplacementBeats)(policy.maxPhaseDisplacementBeats, `${path}.effectPolicy.maxPhaseDisplacementBeats`);
  const participants = array(g.participants, `${path}.participants`, SYNC_ACTION_LIMITS.maxParticipants);
  if (!participants.length) issue(`${path}.participants`, "empty-array", "A complete nonempty roster is required");
  const roster = new Set<string>();
  const leaders = new Set<string>();
  for (const [index, entry] of participants.entries()) {
    const p = `${path}.participants[${index}]`;
    const r = object(entry, p, ["deckId", "syncEnabled", "syncMode", "playing", "audible", "passthrough", "vinylControl", "quantize", "eligibility"]);
    const deck = id(r.deckId, `${p}.deckId`);
    if (roster.has(deck)) issue(`${p}.deckId`, "duplicate-deck", "Sync participants must be distinct");
    roster.add(deck);
    if (!decks.has(deck)) issue(`${p}.deckId`, "sync-roster-mismatch", "Participant lacks a deck precondition");
    const enabled = expected(r.syncEnabled, `${p}.syncEnabled`, bool);
    const mode = expected(r.syncMode, `${p}.syncMode`, (v, q) => oneOf(v, new Set(["none", "follower", "leader-soft", "leader-explicit"]), q));
    if (enabled !== (mode !== "none")) issue(p, "sync-mode-mismatch", "Enabled state must agree with sync mode");
    if (mode === "leader-soft" || mode === "leader-explicit") leaders.add(deck);
    const playing = expected(r.playing, `${p}.playing`, bool);
    const audible = expected(r.audible, `${p}.audible`, bool);
    expected(r.passthrough, `${p}.passthrough`, falseValue);
    expected(r.vinylControl, `${p}.vinylControl`, falseValue);
    expected(r.quantize, `${p}.quantize`, bool);
    const loaded = decks.get(deck)!.binding === "loaded";
    const e = object(r.eligibility, `${p}.eligibility`, loaded ? ["kind", "syncEligible", "beatGridReady", "fileBpm", "effectiveBpm", "rateRatio", "beatPhase"] : ["kind", "syncEligible"]);
    literal(e.kind, loaded ? "loaded" : "empty", `${p}.eligibility.kind`);
    expected(e.syncEligible, `${p}.eligibility.syncEligible`, loaded ? trueValue : falseValue);
    if (loaded) {
      expected(e.beatGridReady, `${p}.eligibility.beatGridReady`, trueValue);
      expected(e.fileBpm, `${p}.eligibility.fileBpm`, bpm);
      expected(e.effectiveBpm, `${p}.eligibility.effectiveBpm`, bpm);
      expected(e.rateRatio, `${p}.eligibility.rateRatio`, range(0, SYNC_ACTION_LIMITS.maxRateRatio, true));
      expected(e.beatPhase, `${p}.eligibility.beatPhase`, range(0, 1, false, true));
    } else if (enabled || playing || audible) issue(p, "active-empty-participant", "An empty peer must be disabled, stopped and silent");
  }
  if (roster.size !== decks.size) issue(path, "sync-roster-mismatch", "Participant and precondition sets must match");
  const required = new Set(["sync-engine", ...Array.from(roster, (deck) => `deck-sync:${deck}`)]);
  if (ownership.size !== required.size || Array.from(required).some((key) => !ownership.has(key))) issue("$.authority.ownership", "sync-ownership-mismatch", "Sync requires exactly one grant per participant and one engine grant");
  if (!g.leader || typeof g.leader !== "object" || Array.isArray(g.leader)) issue(`${path}.leader`, "expected-object", "Expected shared leader identity");
  const leader = g.leader as JsonObject;
  if (leader.kind === "deck") {
    object(leader, `${path}.leader`, ["kind", "deckId"]);
    const deck = id(leader.deckId, `${path}.leader.deckId`);
    if (leaders.size !== 1 || !leaders.has(deck) || decks.get(deck)?.binding !== "loaded") issue(`${path}.leader`, "sync-leader-mismatch", "Named leader must be the sole loaded leader participant");
  } else {
    object(leader, `${path}.leader`, ["kind"]);
    oneOf(leader.kind, new Set(["none", "internal-clock"]), `${path}.leader.kind`);
    if (leaders.size) issue(`${path}.leader`, "sync-leader-mismatch", "Shared leader identity disagrees with participant modes");
  }
}

/** Parses a closed versioned action without resolving authority or reading/mutating host state. */
export function validateSemanticAction(input: unknown): ValidationResult<SemanticAction> {
  try {
    ...
    id(r.sessionId, "$.sessionId"); const actionId = id(r.actionId, "$.actionId"); safeInteger(r.sequence, "$.sequence"); actionVersion(r.schemaVersion, "$.schemaVersion");
    ...
    const sync = r.schemaVersion === 2;
    const decks = freshness(r.preconditions, "$.preconditions", sync);
    ...
    const auth = authority(r.authority, "$.authority", sync);
    if (sync) syncOperation(r.operation, freshnessRecord.conditions, decks, auth.ownership);
    else {
...
export function validateActionOutcome(input: unknown): ValidationResult<ActionOutcome> {
  try {
    ...
    for (const key of ["sessionId", "actionId", "eventId", "clockId"] as const) id(r[key], `$.${key}`); safeInteger(r.sequence, "$.sequence"); actionVersion(r.schemaVersion, "$.schemaVersion"); time(r.atMs, "$.atMs");
