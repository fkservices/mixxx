...
The integrated suite passed 101 tests; TypeScript typecheck and build exited zero on 2026-09-13. Focused integration coverage includes separate-process client exit, stale feedback, capture write failure, socket collision cleanup, file/socket permissions and rejection of oversized or batched requests. A held capture writer test verifies that a newer disarm cancels an older pending arm; independent status remains responsive during the hold. A socket collision preserves the existing server and closes only the rejected service's transport.
...
Mixxx feedback is absent after the endpoint handoff. Native UI inspection timed out repeatedly. A one-second native process sample on 2026-09-13 showed PID 6235 executing its Qt/Cocoa event loop and rendering; no host exit was observed. This does not establish the cause of the UI inspection failure. Restore and verify the mapping and fixture deck state through the isolated native UI, then retain current native evidence before accepting M11. Performance, musical quality, physical-controller coexistence and timing gates remain open.
