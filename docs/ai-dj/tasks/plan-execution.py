...
plan={
 'disclosure':'Autonomously AI-generated reference execution plan; no workers dispatched and no runtime built.',
 'schema_version':1,'catalog_sha256':hashlib.sha256(raw).hexdigest(),
 'authorization':'planning_only','current_task_status':{key:'planned' for key in by},
 'accepted_task_ids':[],'dependency_frontier':['F01'],'dispatchable_now':[],
 ...
 'batches':batches,
 'checkpoints':[dict(id=i,title=t,requires=ids,acceptance=a,status='not_started') for i,t,ids,a in checkpoints],
...
print(('Checked' if args.check else 'Rendered'),len(batches),'reference batches; 171 planned, 0 accepted, 0 dispatched.')
